...
export class SubmissionService {
    ...
    private async generateAutomaticGrade(submission: Submission): Promise<void> {
        try {
            ...
            // Ensure the submission file exists and handle relative paths
            const absoluteFilePath = path.isAbsolute(submission.filePath) 
                ? submission.filePath 
                : path.join(__dirname, '../../', submission.filePath);

            if (!fs.existsSync(absoluteFilePath)) {
                console.error(`Submission file not found at path: ${absoluteFilePath}`);
            ...
            // Ensure the file path is correctly set in the submissions directory
            const fileName = path.basename(submission.filePath);
            const newFilePath = path.join(submissionsDir, fileName);
            
            // Copy the file to the submissions directory if needed
            if (fs.existsSync(submission.filePath) && submission.filePath !== newFilePath) {
                await fs.promises.copyFile(submission.filePath, newFilePath);
                await this.submissionModel.update(submission.id, { filePath: newFilePath });
            ...
            // Calculate average score if grade is an array
            const finalScore = Array.isArray(grade) ? Math.round(grade.reduce((a, b) => a + b, 0) / grade.length) : grade;
    ...
    async submitAndGrade(
        ...
        try {
            ...
            if (exam.status !== 'published') {
                throw new Error('Cannot submit to an unpublished exam');
            }

            // 2. Create submission with proper file handling
            ...
            try {
                ...
                if (file.mimetype === 'application/pdf') {
                    submissionContent = await this.extractTextFromPDF(file.path);
                } else {
                    submissionContent = await fs.promises.readFile(file.path, 'utf8');
                }
                ...
                console.error('Error during grading process:', gradeError);
                // Update submission status to submitted if grading fails
                await this.submissionModel.update(submission.id, {
                    status: 'graded'
                });
                throw new Error(`Failed to grade submission: ${gradeError instanceof Error ? gradeError.message : 'Unknown error'}`);
            }
